...
def predict_debiasing(model, tokenizer, conditioning_model, input_text, args):
...
def main(args):
    ...
    model = MT5ForConditionalGeneration.from_pretrained(args.generation_model, return_dict=True).to(args.device)
    model.eval()
    ...
    results = predict_debiasing(model, tokenizer, conditioning_model, input_text, args)

    print('***')
    for in_text, out_text in zip(input_text, results):
        print('English:', in_text)
        print('Italian:', out_text)
        print('***')
